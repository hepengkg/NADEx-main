...
class MultiHeadSpectralAttention(nn.Module):
    ...
    def forward(self, x,mask=None):
        ...
        返回:
            经过频谱调制和残差连接的张量,形状仍为 (B, seq_len, embed_dim)
        """
        B, N, D = x.shape
        ...
        # if mask is not None:
        #     x_filtered = x_filtered.masked_fill(mask, 0)
        if mask is not None:
            mask = mask.unsqueeze(1).repeat([1, x_filtered.shape[1], 1]).unsqueeze(-1).repeat([1,1,1,x_filtered.shape[-1]])
            x_filtered = x_filtered.masked_fill(mask == 0, -1e9)

        return x + self.dropout(x_filtered)

# TransformerEncoderBlock 模块
...
class Diffu_xstart(nn.Module):
    def __init__(self, hidden_size, args):
        ...
        self.attention_module = MultiHeadSpectralAttention(embed_dim=self.hidden_size, seq_len=65, num_heads=4)
        self.transformer_block = TransformerEncoderBlock(embed_dim=self.hidden_size, attention_module=self.attention_module)
    ...
    def forward(self, rep_item, x_t, t,sr_embs, mask_seq,query_sub3=None):
        ...
        if self.FLE:
            laten_res = self.Frequency_Layer(laten_res)

        laten_res= self.before_diffu_rep(laten_res + emb_t.unsqueeze(1)) #64 65 200

        rep_diffu = self.Transformers(laten_res, mask_seq,query_sub3) #network

        # rep_diffu = self.transformer_block(laten_res,mask = mask_seq)

        # h = rep_diffu[:, -1, :]

        # DFUI = self.Frequency_Layer(laten_res)

        # h = self.alpha * rep_diffu[:, -1, :] + (1 - self.alpha) * DFUI[:, -1, :]

        # if self.diffuser_type == 'mlp1':
        #     res = self.diffuser(torch.cat((x_t, h, emb_t), dim=-1))
        # elif self.diffuser_type == 'mlp2':
        #     res = self.diffuser(torch.cat((x_t, h, emb_t), dim=-1))

        return rep_diffu[:, -1, :]

    def forward_uncon(self, rep_item, x_t, t, sr_embs, mask_seq,query_sub3=None ):
        h = self.none_embedding(torch.tensor([0]).to(rep_item.device))
        h = torch.cat([h.view(1, self.hidden_size)] * x_t.shape[0], dim=0)
        emb_t = self.time_embed(self.timestep_embedding(t, self.hidden_size))
        # laten_res = rep_item.clone()
        #
...
class DiffuRec(nn.Module):
    def __init__(self, args,):
        ...
        self.w = 3
        # self.xstart_model_unh = self.xstart_model.forward_uncon()
    ...
    def p_mean_variance(self, item_rep, x_t, t,sr_embs, mask_seq,query_sub3=None):
        # model_output_uncon = self.xstart_model.forward_uncon(item_rep, x_t, self._scale_timesteps(t),sr_embs, mask_seq,query_sub3)
        model_output_con = self.xstart_model(item_rep, x_t, self._scale_timesteps(t),sr_embs, mask_seq,query_sub3)
        ...
        # model_output = (1 + self.w) * model_output_con - self.w * model_output_uncon
        model_output = model_output_con 
        return model_output, []
    ...
    def forward(self, item_rep, item_tag,sr_embs, mask_seq,t,query_sub3=None):        
        ...
        x_0 = self.xstart_model(item_rep, item_rep[:,-1,:], self._scale_timesteps(t),sr_embs, mask_seq,query_sub3)  ##output predict
