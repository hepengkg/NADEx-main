...
from model_dreamrec import create_model_diffu, Att_Diffuse_model
...
def test(args, model, model_name,
    ...
    with torch.no_grad():
        for time_idx, test_snap in enumerate(tqdm(test_list,colour='RED')):
            ...
            sequence = np.load('/mnt/sdb/user_data/gyl/DiffuTKG/src/data_cyx/{}/history_seq/h_r_seen_triple_{}.npy'.format(args.dataset,str(query_time))
            ...
            if args.add_static_graph:
                all_tail_seq = sp.load_npz('/mnt/sdb/user_data/gyl/DiffuTKG/src/data_cyx/{}/history_seq/h_r_history_seq_{}.npz'.format(args.dataset, str(query_time)))  # 统计历史事实是否出现
            ...
            diffu_reps = []
            for seed in range(1):
                scores, diffu_rep, weights, t, _,ent_emb,_ = model(sequence,true_triples, False, use_cuda
...
def write_pred_text(pred,ground,test_snap,time,d,epoch):
    file_path = "/mnt/sdb/user_data/gyl/DiffuTKG/results/pred_result/{}/pred_{}".format(d, time)
    # 确保路径存在，如果路径不存在则自动创建
...
def run_experiment(args):
    ...
    # models
    model_path = './models/{}_seed_{}_lamba_{}_gamma_{}_FLE_{}_c_{}tau{}_scale_{}_len_{}'.format(args.dataset,args.seed,args.lamda,args.gamma,args.FLE,args.c,args.temperature_object,args.scale,args.max_len)
    ...
    if args.add_static:
        static_triples = np.array(_read_triplets_as_list("/mnt/sdb/user_data/gyl/DiffuTKG/data/" + args.dataset + "/e-w-graph.txt", load_time=False))
    ...
    if args.test:
        ...
        for epoch in range(args.n_epochs):
            ...
            for batch_idx, train_sample_num in enumerate(tqdm(idx)):
                ...
                sequence = np.load('/mnt/sdb/user_data/gyl/DiffuTKG/src/data_cyx/{}/history_seq/h_r_seen_triple_{}.npy'.format(args.dataset,str(train_times[train_sample_num]))
                ...
                seq_idx = true_triples[:, 0] * num_rels * 2 + true_triples[:, 1] 
                if args.add_static_graph:
                    all_tail_seq = sp.load_npz('/mnt/sdb/user_data/gyl/DiffuTKG/src/data_cyx/{}/history_seq/h_r_history_seq_{}.npz'.format(args.dataset, str(train_times[train_sample_num])))  # 统计历史事实是否出现
                    history_tail_seq = torch.Tensor(all_tail_seq[seq_idx.cpu()].todense())
                    one_hot_tail_seq = history_tail_seq.masked_fill(history_tail_seq != 0, 1)
                    if use_cuda:
                        history_tail_seq, one_hot_tail_seq = history_tail_seq.to(args.gpu), one_hot_tail_seq.to(args.gpu)
                ...
                scores, diffu_rep, weights, t, mask_seq,emb_ent,loss_neg = model(sequence,true_triples, True, use_cuda,history_tail_seq,seen_before_label,static_graph)
                ...
                # loss = (loss_diffu_value-loss_neg) / accumulation_steps  # 将损失除以累积次数，这样使得每次累积的梯度相当于一个较大批次的梯度
                # loss = 0.9 * loss_diffu_value - 0.1 * loss_neg
                #print("loss_diffu_value: ",loss_diffu_value)
                #print("loss_neg: ",loss_neg)
                #print("loss: ",loss)
                # model_diff = (1 - args.alph)*loss_diffu_value - args.alph * loss_neg
                model_diff = loss_diffu_value - loss_neg
                loss = -(1 - args.lamda) * F.logsigmoid(-args.gamma * model_diff + 1e-8) + loss_diffu_value * args.lamda
                loss = loss / accumulation_steps  # 将损失除以累积次数，这样使得每次累积的梯度相当于一个较大批次的梯度
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_dropout', type=float, default=0.1, help='Dropout of embedding')
    ...
    # parser.add_argument('--beta_end', type=float, default=0.002, help='beta_end weight')
    parser.add_argument('--beta_end', type=float, default=0.02, help='beta_end weight')
    parser.add_argument("--c", default=10, type=int)
    parser.add_argument("--FLE", default=False, type=str)

    parser.add_argument("--lamda", default=0.8, type=float)
    parser.add_argument("--gamma", default=7.0, type=float)
    # parser.add_argument("--alph", default= 0.2, type=float, help="loss balance")

    args = parser.parse_args()
    print(args)
    fitlog.set_log_dir("logs/data_{}".format(args.dataset))         # 设定日志存储的目录
