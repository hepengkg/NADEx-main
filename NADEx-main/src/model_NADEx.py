...
import torch
import math
from difffu_dreamrec import DiffuRec
...
class Att_Diffuse_model(nn.Module):
    ...
    def diffu_pre(self, item_rep, tag_emb, sr_embs, mask_seq,t,query_sub3=None):
        seq_rep_diffu = self.diffu(item_rep, tag_emb,sr_embs, mask_seq,t,query_sub3)
    ...
    def diffu_rep_pre(self, rep_diffu,e_embs,history_tail_seq=None, one_hot_tail_seq=None):
        ...
        scores = (rep_diffu[0]) @ e_embs[:-1].t()/(math.sqrt(self.emb_dim)*self.temperature_object)#-9,14  328 7128
    ...
    def in_batch_negative_sampling(self, batch_items):
    ...
    def cosine_loss(self,pred_matrix, gt_matrix):
        ...
        # loss = (1 - (x * y).sum(dim=-1)).pow_(2)
        
        # loss = loss.mean()
        return loss

    
    def forward(self, sequence, tag, train_flag=True,use_cuda=True,history_tail_seq = None,seen_before_label=None,static_graph=None):
        ...
        if train_flag: ### 训练过程
            ...
            # query_object_noise = self.diffu.q_sample(query_object3, t)  # 改
            ...
            #             labels_neg = self.in_batch_negative_sampling(tag[:,2])
            
            labels_neg = self.in_batch_negative_sampling(tag[:,2])
            x_start_neg_ = torch.mean(labels_neg.float(), dim=1)
            query_object3_neg = initial_h[x_start_neg_.to(torch.long)]
            x_start_neg = query_object3_neg.unsqueeze(1)
            ...
            query_object_noise = self.diffu.q_sample(query_object3, t)  # 改
            x_noisy_neg = self.diffu.q_sample(x_start_neg, t)
            object_embeddings_or = object_embeddings.clone()

            object_embeddings = torch.concat([object_embeddings,query_object_noise],dim=1)
            # inter_embeddings = object_embeddings+time_embeddings_re+relation_embeddings
            inter_embeddings = object_embeddings

            inter_embeddings_drop = self.LayerNorm(self.embed_dropout(inter_embeddings)) ## dropout first than layernorm
            rep_diffu = self.diffu_pre(inter_embeddings_drop, inter_embeddings_drop[:,-1,:],initial_h, mask_seq,t)

            object_embeddings_neg = torch.concat([object_embeddings_or,x_noisy_neg],dim=1)
            inter_embeddings_neg = object_embeddings_neg
            # inter_embeddings_neg = object_embeddings_neg+time_embeddings_re+relation_embeddings
            inter_embeddings_drop_neg = self.LayerNorm(self.embed_dropout(inter_embeddings_neg)) ## dropout first than layernorm
            predicted_x_neg = self.diffu_pre(inter_embeddings_drop_neg, inter_embeddings_drop_neg[:, -1, :], initial_h, mask_seq, t)

            loss_neg = self.cosine_loss(x_start_neg.squeeze(1), predicted_x_neg)

            rep_item,object_protype_gt = None,None
        else:
            noise_x_t = th.randn_like(object_embeddings[:,[0,1,-1],:]) #328 3 200
            
            query_object3 = noise_x_t[:,[2],:] #328 1 200
            object_embeddings = torch.concat([object_embeddings,query_object3],dim=1)#328 65 200
            inter_embeddings = object_embeddings+time_embeddings_re + relation_embeddings#328 65 200
            inter_embeddings_drop = self.LayerNorm(self.embed_dropout(inter_embeddings)) ## #328 65 200 dropout first than layernorm
        ...
        return scores, rep_diffu, rep_item, t, mask_seq,initial_h,loss_neg
