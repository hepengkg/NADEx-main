...
def process_triples(matrix, max_sequence_length=-1,m=-1,r=-1,history_times=-1):
    ...
    for ti in trange(0,t):
        if ti == 0:
            np.save('F:\code\TKG\DiffuTKG-main\src\data/{}/history_seq/h_r_seen_triple_{}.npy'.format(dataset,str(history_times[ti])), sparse.csr_matrix((sequences_dict+1)[matrix[ti][:,0]].reshape(len(matrix[ti]),-1)))
            continue
        ...
        np.save('F:\code\TKG\DiffuTKG-main\src\data/{}/history_seq/h_r_seen_triple_{}'.format(dataset,str(history_times[ti])), sparse.csr_matrix((sequences_dict+1)[matrix[ti][:,0]].reshape(len(matrix[ti]),-1)))
for dataset in ['ICEWS14','ICEWS18','GDELT','ICEWS05- 15']:
